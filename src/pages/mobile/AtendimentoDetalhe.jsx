...
function fmtHora(str) { return str ? str.substring(11,16) : '—' }
...
function TotalDiretoInput({ onSalvar }) {
  ...
  return (
    <div className="flex gap-2">
      <input type="number" value={valor} onChange={e => setValor(e.target.value)}
        ...
        step="0.01" placeholder="Ex: 150,00" />
      <button onClick={() => valor && onSalvar(valor)} disabled={!valor}
...
export default function AtendimentoDetalhe() {
  ...
  async function buscar() {
    ...
    setServico(s); setPecas(p||[])
    setDiagnostico(s?.diagnosis||''); setTrabalho(s?.work_done||''); setMaoDeObra(s?.labor_price||'')
    setLoading(false)
  }
  ...
  async function iniciarAtendimento() {
    ...
    await buscar(); setSalvando(false)
  }
  ...
  async function salvarMaoDeObra() {
    ...
    await supabase.from('services').update({ labor_price: val||null, total_price: (val+totalPecas)||null }).eq('id', id)
    setEditandoMao(false); buscar()
  }
  ...
  async function adicionarPeca() {
    ...
    const totalPecas = [...pecas, {quantity: novaPeca.quantity, unit_price: preco}].reduce((s,p) => s+(p.quantity*p.unit_price), 0)
    await supabase.from('services').update({ total_price: totalPecas + parseFloat(servico?.labor_price||0) }).eq('id', id)
    setNovaPeca({ description:'', quantity:1, unit_price:'' }); setAdicionandoPeca(false); buscar()
  }
  async function removerPeca(pecaId) {
    await supabase.from('service_parts').delete().eq('id', pecaId); buscar()
  }
  function abrirMaps() {
    const c = servico?.clients; if (!c) return
    window.open(`https://maps.google.com/?q=${encodeURIComponent([c.address,c.neighborhood,c.city].filter(Boolean).join(', '))}`, '_blank')
  }

  if (loading) return <div className="min-h-screen flex items-center justify-center"><div className="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin" /></div>
  if (!servico) return <div className="min-h-screen flex items-center justify-center text-gray-500 text-sm">Atendimento não encontrado.</div>
  ...
  const agendado = servico.status === 'agendado'
  ...
  return (
    <div className="bg-gray-50" style={{minHeight: '100dvh', display: 'flex', flexDirection: 'column'}}>
        ...
        <div className="px-4 space-y-3 mt-4 pb-4">
          ...
          <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
            ...
            {cliente.phone && (
              <a href={`tel:${cliente.phone}`} className="flex items-center gap-3 px-4 py-3 border-b border-gray-50 active:bg-gray-50">
                ...
                <div><div className="text-sm font-medium text-gray-900">{cliente.phone}</div><div className="text-xs text-gray-400">Toque para ligar</div></div>
              </a>
            )}
            {(cliente.address||cliente.neighborhood||cliente.city) && (
              <button onClick={abrirMaps} className="w-full flex items-center gap-3 px-4 py-3 active:bg-gray-50 text-left">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-gray-900 truncate">{[cliente.address,cliente.neighborhood].filter(Boolean).join(', ')}</div>
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-gray-50">
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                <div><div className="text-xs text-gray-400">Agendado para</div><div className="text-sm font-medium text-gray-900">{fmtData(servico.scheduled_at)} às {fmtHora(servico.scheduled_at)}</div></div>
              </div>
              <div className="flex items-start gap-3 px-4 py-3">
                <Wrench size={16} className="text-gray-400 flex-shrink-0 mt-0.5" />
                <div><div className="text-xs text-gray-400">Tipo</div><div className="text-sm font-medium text-gray-900">{servico.type||'—'}</div></div>
              </div>
              {servico.problem && <div className="px-4 py-3"><div className="text-xs text-gray-400 mb-1">Problema relatado</div><div className="text-sm text-gray-700 leading-relaxed">{servico.problem}</div></div>}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-50">
              ...
              {podeEditar && !editandoDiagnostico && <button onClick={() => setEditandoDiagnostico(true)} className="p-1.5 rounded-lg bg-gray-100"><Edit2 size={14} className="text-gray-500" /></button>}
            </div>
            <div className="px-4 py-3">
              {editandoDiagnostico ? (
                <div className="space-y-2">
                  <textarea value={diagnostico} onChange={e => setDiagnostico(e.target.value)} rows={4} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-primary resize-none" autoFocus />
                  <div className="flex gap-2">
              ...
              ) : <p className="text-sm text-gray-700 leading-relaxed">{servico.diagnosis || <span className="text-gray-300 italic">Nenhum diagnóstico registrado</span>}</p>}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-50">
              ...
              {podeEditar && !editandoTrabalho && <button onClick={() => setEditandoTrabalho(true)} className="p-1.5 rounded-lg bg-gray-100"><Edit2 size={14} className="text-gray-500" /></button>}
            </div>
            <div className="px-4 py-3">
              {editandoTrabalho ? (
                <div className="space-y-2">
                  <textarea value={trabalho} onChange={e => setTrabalho(e.target.value)} rows={4} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-primary resize-none" autoFocus />
                  <div className="flex gap-2">
              ...
              ) : <p className="text-sm text-gray-700 leading-relaxed">{servico.work_done || <span className="text-gray-300 italic">Nenhum trabalho registrado</span>}</p>}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-50">
              ...
              {podeEditar && <button onClick={() => setAdicionandoPeca(true)} className="flex items-center gap-1 bg-navy/10 text-navy rounded-lg px-2.5 py-1.5 text-xs font-semibold"><Plus size={13} /> Adicionar</button>}
            </div>
            {adicionandoPeca && (
              <div className="px-4 py-3 bg-gray-50 border-b border-gray-100 space-y-2">
                <input value={novaPeca.description} onChange={e => setNovaPeca(p=>({...p,description:e.target.value}))} placeholder="Nome da peça" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-primary bg-white" autoFocus />
                <div className="grid grid-cols-2 gap-2">
                  <input type="number" value={novaPeca.quantity} onChange={e => setNovaPeca(p=>({...p,quantity:parseInt(e.target.value)||1}))} placeholder="Qtd" min={1} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-primary bg-white" />
                  <input type="number" value={novaPeca.unit_price} onChange={e => setNovaPeca(p=>({...p,unit_price:e.target.value}))} placeholder="Valor unitário" step="0.01" className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-primary bg-white" />
                </div>
                <div className="flex gap-2">
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-50">
              ...
              {podeEditar && !editandoMao && <button onClick={() => setEditandoMao(true)} className="p-1.5 rounded-lg bg-gray-100"><Edit2 size={14} className="text-gray-500" /></button>}
            </div>
            {editandoMao ? (
              <div className="px-4 py-4 space-y-3">
                <p className="text-xs text-gray-400">Preencha <strong>mão de obra</strong> para discriminar, ou <strong>total direto</strong> para valor único:</p>
                {totalPecas > 0 && <div className="flex justify-between text-sm"><span className="text-gray-500">Peças</span><span className="font-medium">{fmt(totalPecas)}</span></div>}
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">Mão de obra (opcional)</label>
                  <input type="number" value={moOrHand} onChange={e => setMaoDeObra(e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-primary" step="0.01" placeholder="0,00" />
                </div>
                <div className="border-t border-dashed border-gray-200 pt-3">
                  <label className="text-xs text-gray-500 mb-1 block">Ou preencha só o total (sem discriminar)</label>
                  <TotalDiretoInput onSalvar={salvarTotalDireto} />
                </div>
              ...
              <div className="divide-y divide-gray-50">
                {totalPecas > 0 && <div className="flex justify-between items-center px-4 py-3"><span className="text-sm text-gray-600">Peças</span><span className="text-sm font-medium text-gray-900">{fmt(totalPecas)}</span></div>}
                {servico.labor_price > 0 && <div className="flex justify-between items-center px-4 py-3"><span className="text-sm text-gray-600">Mão de obra</span><span className="text-sm font-medium text-gray-900">{fmt(servico.labor_price)}</span></div>}
                <div className="flex justify-between items-center px-4 py-3.5 bg-gray-50"><span className="text-sm font-bold text-gray-900">Total</span><span className="text-base font-bold text-navy">{fmt(servico.total_price||totalGeral)}</span></div>
              </div>
            )}
          </div>
      ...
      {/* BOTÕES DE AÇÃO — parte do flex, não fixed */}
      <div className="bg-white border-t border-gray-100 px-4 py-4 flex-shrink-0" style={{paddingBottom: 'max(1rem, env(safe-area-inset-bottom))'}}>

        {agendado && (
          <div className="flex gap-3">
            <button onClick={iniciarAtendimento} disabled={salvando}
              className="flex-1 bg-navy text-white rounded-2xl py-4 font-bold text-sm disabled:opacity-60">
              {salvando ? '...' : '▶ Iniciar'}
            </button>
            <button onClick={concluirAtendimento} disabled={salvando}
              className="flex-1 bg-green-600 text-white rounded-2xl py-4 font-bold text-sm disabled:opacity-60 flex items-center justify-center gap-1">
              <CheckCircle size={18} />
              {salvando ? '...' : 'Concluir'}
            </button>
          </div>
        ...
        {concluido && (
          <div className="w-full bg-green-50 text-green-700 rounded-2xl py-4 font-semibold text-sm text-center flex items-center justify-center gap-2">
            <CheckCircle size={18} /> Concluído em {fmtData(servico.finished_at)}
          </div>
        )}
