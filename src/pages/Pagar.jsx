...
const CATEGORIAS = ['Fornecedor', 'Aluguel', 'Salário', 'Impostos', 'Serviços', 'Equipamentos', 'Peças', 'Outros']
...
export default function Pagar() {
  ...
  const [formConta, setFormConta] = useState({ description:'', amount:'', due_date:'', category:'' })
  ...
  const filtradas = contas.filter(c => {
    if (busca) {
      ...
      if (!desc.includes(busca.toLowerCase())) return false
    }
  ...
  function abrirNova() {
    setFormConta({ description:'', amount:'', due_date: hoje(), category:'' })
    setModalConta('novo')
  }
  function abrirEditar(c) {
    setFormConta({ description: c.description||'', amount: c.amount||'', due_date: c.due_date||'', category: c.category||'' })
    setModalConta(c)
  }
            ...
            <div key={c.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
              <div className="flex items-start gap-3 px-4 py-3">
                <div className={`w-1 self-stretch rounded-full flex-shrink-0 ${
                  ...
                  stNorm(c) === 'pago' ? 'bg-green-400' : 'bg-blue-400'
                }`} />
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    ...
                    {c.status === 'pago' && c.paid_at && (
                      <span className="text-xs text-green-600">Pago: {fmtData(c.paid_at)}</span>
                    )}
                  </div>
                  {c.payment_method && (
                    <p className="text-xs text-gray-400 mt-0.5">{c.payment_method}</p>
                  )}
                </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end">
          <div className="bg-white w-full rounded-t-3xl p-5 space-y-4"
            ...
            <div className="bg-gray-50 rounded-xl p-3">
              <p className="text-sm font-semibold text-gray-900 truncate">{modalPagar.description}</p>
          ...
          <div className="bg-white w-full rounded-t-3xl p-5 space-y-4 max-h-[90vh] overflow-y-auto"
            ...
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-navy">
                {modalConta === 'novo' ? 'Nova Conta a Pagar' : 'Editar Conta'}
              </h3>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {CATEGORIAS.map(cat => (
                  <button key={cat} onClick={() => setFormConta(f=>({...f,category:cat}))}
                    className={`py-2 px-1 rounded-xl text-xs font-medium border transition text-center ${
                      formConta.category === cat ? 'bg-navy text-white border-navy' : 'bg-white text-gray-600 border-gray-200'
                    }`}>{cat}</button>
                ))}
              </div>
            </div>
